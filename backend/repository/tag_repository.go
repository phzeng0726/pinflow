...
func (r *tagRepository) CreateOrGet(boardID uint, name string, color string) (*model.Tag, error) {
	name = strings.TrimSpace(name)

	// Check if tag already exists in this board (case-insensitive)
	existing, err := r.s.GetTagByName(boardID, name)
	if err == nil {
		return existing, nil
	}
...
func (r *tagRepository) ListByBoard(boardID uint) ([]model.Tag, error) {
	return r.s.GetTagsByBoard(boardID), nil
}
...
func (r *tagRepository) FindByName(boardID uint, name string) (*model.Tag, error) {
	return r.s.GetTagByName(boardID, name)
}
