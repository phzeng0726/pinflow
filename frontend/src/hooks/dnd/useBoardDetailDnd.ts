...
interface UseBoardDetailDndParams {
  boardId: number
  columns: Column[]
...
export function useBoardDetailDnd(params: UseBoardDetailDndParams) {
  const {
    boardId,
    columns,
    moveColumnMutate,
  ...
  const setBoardDetailCache = (updater: (board: Board) => Board) => {
