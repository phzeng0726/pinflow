import type { ArchivedColumn } from '@/types'
...
export function ArchivedColumnItem({ column, onRestore, onDelete, isRestoring, disabled }: Props) {
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-1.5">
          <span className="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-300">
            {t('archive.cardCount', { count: column.cardCount })}
          </span>
        </div>
        <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">
          {new Date(column.archivedAt).toLocaleDateString()}
        </p>
      </div>
