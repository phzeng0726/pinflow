...
const EDGE_LEGEND_ITEMS: { type: DependencyType; labelKey: RelationLabelKey }[] = [
...
const URGENCY_LEGEND_ITEMS: { labelKey: UrgencyLabelKey; colorKey: keyof typeof URGENCY_HEX_COLORS }[] = [
...
export function GraphLegend() {
  ...
  return (
    <div className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2">
      <div className="flex flex-col items-center gap-2 rounded-xl border bg-white/90 px-4 py-2 shadow-md backdrop-blur-sm dark:border-gray-700 dark:bg-gray-800/90 sm:flex-row">
        {/* Edge style legend */}
        <div className="flex items-center gap-3 whitespace-nowrap">
          {EDGE_LEGEND_ITEMS.map(({ type, labelKey }) => {
            const { stroke, strokeDasharray, markerEnd } = DEPENDENCY_EDGE_STYLES[type]
            return (
        ...
        <div className="hidden h-5 w-px bg-gray-200 dark:bg-gray-600 sm:block" />
