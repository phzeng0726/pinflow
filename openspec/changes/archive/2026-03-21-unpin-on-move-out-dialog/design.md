...
- 用戶選擇「僅移出」時，不改變釘選狀態
- Dialog 僅在「來源為 auto-pin column」且「卡片目前 is_pinned=true」時出現
...
- **[Risk] column 資料不含 auto_pin**：drag-end 時需要 column 的 `auto_pin` 欄位 → Mitigation：確認 `useColumns` hook 回傳資料包含此欄位；若無，補充 column type
