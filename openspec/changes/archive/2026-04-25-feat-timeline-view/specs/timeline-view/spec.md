...
每張有 `startTime` 且有 `endTime` 的卡片 SHALL 在畫布上渲染一個甘特 bar，bar 的 `left` 與 `width` 根據日期與目前 zoom 的欄寬計算。Bar 顏色 SHALL 對應所屬 Column；bar 內 SHALL 顯示卡片標題（overflow ellipsis）與 checklist 完成度（e.g., `3/8`）。Bar 點擊 SHALL 開啟 `CardDetailDialog`。
...
### Requirement: 無排程卡片顯示
未同時設定 `startTime` 與 `endTime` 的卡片（僅有其中一個日期、或兩者皆無）SHALL 統一顯示於畫布底部「No dates」群組，以虛線框 bar 呈現（固定 120px 寬），不根據日期定位。
...
### Requirement: 標題搜尋
Toolbar SHALL 提供搜尋輸入框，輸入後即時（無需按 Enter）過濾卡片列，僅保留標題包含搜尋字串（大小寫不敏感）的卡片。搜尋不影響分組結構（空群組可隱藏）。

#### Scenario: 即時搜尋過濾
- **WHEN** 使用者在搜尋框輸入 `bug`
- **THEN** 只有標題含 `bug`（不分大小寫）的卡片列顯示，其餘列隱藏
...
#### Scenario: 搜尋後空群組隱藏
- **WHEN** 使用者搜尋後，`by-status` 分組下某 Column 的所有卡片均不符合篩選條件
- **THEN** 該 Column 的 lane 標頭列自動隱藏，不顯示空群組
...
#### Scenario: Hover 模式高亮
- **WHEN** depMode 為 `hover` 且使用者 hover 某卡片
- **THEN** 與該卡片直接相連的依賴線 opacity=1；其餘線 opacity=0.1；相關卡片列正常顯示，無關卡片列 opacity=0.15

#### Scenario: Hover 模式無 hover
- **WHEN** depMode 為 `hover` 且無卡片被 hover
- **THEN** 所有依賴線以低 opacity（0.25）顯示，所有卡片列正常顯示

---

### Requirement: 依賴線型別篩選
Toolbar SHALL 提供篩選按鈕，點擊後顯示 filter panel，可獨立開關 `blocks`、`parent_of`、`duplicates`、`related_to` 四種依賴型別的線條顯示。關閉某型別後，該型別的所有線條立即從畫布消失。

#### Scenario: 關閉 blocks 型別
- **WHEN** 使用者在 filter panel 取消勾選 `blocks`
- **THEN** 所有 blocks 型別的依賴線從畫布移除，其他型別不受影響
