...
The system SHALL allow users to create Markdown-formatted comments on a card via a `MarkdownEditor` in the `CommentSection`. When the create area is collapsed, clicking it SHALL expand and render a `MarkdownEditor` (replacing the previous textarea). The backend SHALL automatically associate the comment with the current workspace's `workspaceId` as `authorId`; the client SHALL NOT send `authorId` in the request body.
